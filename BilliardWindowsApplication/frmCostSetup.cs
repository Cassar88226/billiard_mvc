...
namespace BilliardWindowsApplication
{
    public partial class frmCostSetup : Form
    {
       ...
       async void dayscost(int nodays)
		...
		private string GetFormattedString(string src, string suffix)
		{
			double dblValue = 0.0;
			src = Regex.Replace(src, "[^0-9.,]", string.Empty);
			src.Replace(",", ".");
			if (double.TryParse(src, NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture, out dblValue))
			{
				src = dblValue.ToString("0.00", CultureInfo.InvariantCulture);
			}
			src = src.Replace(".", ",");
			return src + suffix;
		}
        void hideAll()
        {
            try
            {
                ...
                if (textBox1.Visible)
                {
                    lblh1.Text = GetFormattedString(textBox1.Text, "€");
                    textBox1.Visible = false;
                }
                if (textBox2.Visible)
                {
					lblh2.Text = GetFormattedString(textBox2.Text, "€");
                    textBox2.Visible = false;
                }
                if (textBox3.Visible)
                {
					lblh3.Text = GetFormattedString(textBox3.Text, "€");
                    textBox3.Visible = false;
                }
                if (textBox4.Visible)
                {
					lblh4.Text = GetFormattedString(textBox4.Text, "€");
