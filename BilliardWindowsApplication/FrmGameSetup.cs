...
namespace BilliardWindowsApplication
{
    public partial class FrmGameSetup : Form
    {
        ...
        async void insertgamecost()
        {
            ...
            {
				BLL_BilliardWindowsApplication.WriteLog("loadcostdetails ----> checklastgamecost()");
				try
				{
            ...
            else
            {
                ...
                else    //added for clearing previous scoredata for this id
                {
                    string result = await Task.Run(() => API.deleteScoreData(BLL_BilliardWindowsApplication.gamecostdetailsStatic.id));
                }
            }
        ...
        void frm_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            {
				while (API.UpdatePlayerLogin(BLL_BilliardWindowsApplication.player1.PlayerId, "0") == false.ToString())
				{
					Thread.Sleep(200);
				}
				preplayer.Add(BLL_BilliardWindowsApplication.player1.PlayerId);
			}
            if (!string.IsNullOrEmpty(BLL_BilliardWindowsApplication.player2.Name))
			{
				while (API.UpdatePlayerLogin(BLL_BilliardWindowsApplication.player2.PlayerId, "0") == false.ToString())
				{
					Thread.Sleep(200);
				}
				preplayer.Add(BLL_BilliardWindowsApplication.player2.PlayerId);
			}
            if (!string.IsNullOrEmpty(BLL_BilliardWindowsApplication.player3.Name))
			{
				while (API.UpdatePlayerLogin(BLL_BilliardWindowsApplication.player3.PlayerId, "0") == false.ToString())
				{
					Thread.Sleep(200);
				}
				preplayer.Add(BLL_BilliardWindowsApplication.player3.PlayerId);
			}
            if (!string.IsNullOrEmpty(BLL_BilliardWindowsApplication.player4.Name))
			{
				while (API.UpdatePlayerLogin(BLL_BilliardWindowsApplication.player4.PlayerId, "0") == false.ToString())
				{
					Thread.Sleep(200);
				}
				preplayer.Add(BLL_BilliardWindowsApplication.player4.PlayerId);
			}
            ...
            {              
                ...
                BallTrackAPI.ptCorners = new POINT[6];
                BallTrackAPI.ptCorner_new = new POINT[6];
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(BLL_BilliardWindowsApplication.player1.Name))
                    {
                        if (BLL_BilliardWindowsApplication.player1.ClubId.Trim() == "0")
                        {
							...
							pbPlayer1.ImageLocation = BLL_BilliardWindowsApplication.originalLocation + BLL_BilliardWindowsApplication.player1.PlayerPicture;
							pbClub1.ImageLocation = BLL_BilliardWindowsApplication.originalLocation + BLL_BilliardWindowsApplication.player1.ClubPicture;
                        }
                        else if (MessageBox.Show("You are already logged in." + Environment.NewLine + "Do you want to end previous game?", "Logout", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
						{
							while (API.UpdatePlayerLogin(BLL_BilliardWindowsApplication.player1.PlayerId, "0") == false.ToString())
							{
								Thread.Sleep(200);
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(BLL_BilliardWindowsApplication.player2.Name))
                    {
                        if (BLL_BilliardWindowsApplication.player2.ClubId.Trim() == "0")
                        {
							...
							pbPlayer2.ImageLocation = BLL_BilliardWindowsApplication.originalLocation + BLL_BilliardWindowsApplication.player2.PlayerPicture;
							pbClub2.ImageLocation = BLL_BilliardWindowsApplication.originalLocation + BLL_BilliardWindowsApplication.player2.ClubPicture;
                        }
						else if (MessageBox.Show("You are already logged in." + Environment.NewLine + "Do you want to end previous game?", "Logout", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
						{
							while (API.UpdatePlayerLogin(BLL_BilliardWindowsApplication.player2.PlayerId, "0") == false.ToString())
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(BLL_BilliardWindowsApplication.player3.Name))
                    {
                        if (BLL_BilliardWindowsApplication.player3.ClubId.Trim() == "0")
                        {
							...
							pbPlayer3.ImageLocation = BLL_BilliardWindowsApplication.originalLocation + BLL_BilliardWindowsApplication.player3.PlayerPicture;
							pbClub3.ImageLocation = BLL_BilliardWindowsApplication.originalLocation + BLL_BilliardWindowsApplication.player3.ClubPicture;
                        }
						else if (MessageBox.Show("You are already logged in." + Environment.NewLine + "Do you want to end previous game?", "Logout", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
						{
							while (API.UpdatePlayerLogin(BLL_BilliardWindowsApplication.player3.PlayerId, "0") == false.ToString())
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(BLL_BilliardWindowsApplication.player4.Name))
                    {
                        if (BLL_BilliardWindowsApplication.player4.ClubId.Trim() == "0")
                        {
							...
							pbPlayer4.ImageLocation = BLL_BilliardWindowsApplication.originalLocation + BLL_BilliardWindowsApplication.player4.PlayerPicture;
							pbClub4.ImageLocation = BLL_BilliardWindowsApplication.originalLocation + BLL_BilliardWindowsApplication.player4.ClubPicture;
                        }
						else if (MessageBox.Show("You are already logged in." + Environment.NewLine + "Do you want to end previous game?", "Logout", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
						{
							while (API.UpdatePlayerLogin(BLL_BilliardWindowsApplication.player4.PlayerId, "0") == false.ToString())
