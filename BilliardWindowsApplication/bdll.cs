...
namespace BilliardWindowsApplication
{
    ...
    {
		public static biliardService.BilliardScoreboard API = new biliardService.BilliardScoreboard();
        ...
        public static POINT[] ptCorners = new POINT[6];
        public static POINT[] ptCorner_new = new POINT[6];
		...
		public static double m_fMillPerDot;
		...
		public static string m_strWebDownloadPath = "";
        ...
        [DllImport(strPath, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
        public static extern double BTAPI_GetAveSpeedForReplay(int nPlayer, int nShot);
        [DllImport(strPath, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
        public static extern double BTAPI_GetAveSpeedForRealTime();
		...
		public static POINT ScalePoint(POINT pt, double fScaleX, double fScaleY, int nMarginX, int nMarginY)
		{
		...
		public static void ScaleCurrentDrawInfo(double fScaleX, double fScaleY, int nMarginX, int nMarginY, int nClipWidth, int nClipHeight)
		{
		...
		public static bool InitSDK(IntPtr proc, IntPtr hwndMain, IntPtr hwndTracking, IntPtr hwndCamera)
		{
			...
			if (bRet)
			{
				...
				m_fMillPerDot = (double)2840.0f / (double)(g_rcTable.right - g_rcTable.left);
			}
		...
		// 	}
			...
			if (nBallIndex == 0 && m_yellowBallInfo.vptTrajectory.Count() > 0 && m_redBallInfo.vptTrajectory.Count() > 0)		//white ball
			{
				...
				nDist[4] = (int)Math.Sqrt((double)((ptNew.x - ptBall1.x) * (ptNew.x - ptBall1.x) + (ptNew.y - ptBall1.y) * (ptNew.y - ptBall1.y)));
				nDist[5] = (int)Math.Sqrt((double)((ptNew.x - ptBall2.x) * (ptNew.x - ptBall2.x) + (ptNew.y - ptBall2.y) * (ptNew.y - ptBall2.y)));
			}
			...
			if (nMinVal < BALL_RADIUS_REAL * 3)
			{
				...
				if (pBallInfo.nDist[nMinIndex] < nDist[nMinIndex] && pBallInfo.nDist[nMinIndex] < BALL_RADIUS_REAL)		//crashed
				{
					...
					{
						m_nHitRailCount++;
						if (pBallInfo.nDist[nMinIndex] <= BALL_RADIUS_REAL)// || (pBallInfo.nDist[nMinIndex] <= 20 && (nDist[nMinIndex] - pBallInfo.nDist[nMinIndex]) > 20))
						{
			...
			if (m_nHitRailCount == m_nRailCount && m_nHitRailCount > 0)
			{
				if (m_bShowTeacherPoint)
				{
					...
					if (pCurBallInfo.nLastOutputCandidate >= 0 && pCurBallInfo.vIndexCrashed.Count() > pCurBallInfo.nLastOutputCandidate)
					{
						...
						if (GetDotDistance(ptLast, element.pt) >= 8)		//20 -> 8, for corner impact
						{
				...
				if (!m_bStrikedTargetBall)		//impacted red ball
				{
					if (nBallIndex == 1 - m_nCurrentPlayer)		//ball has been impacted 
					{
						if (pPlayerBallInfo.fAveSpeed == 0.0f)
						{
                            ...
                            pPlayerBallInfo.fAveSpeed = BTAPI_GetAveSpeedForRealTime(); //pPlayerBallInfo.fTotalSpeed / (double)pPlayerBallInfo.nSpeedRecordCount;
						}
		...
		public static void EstimateBallSpeed(BALL_TRACK_INFO pBallInfo, int nBallIndex)
		{
			int i;
			{
				double fTotalMovedDistance = 0.0f;
				for (i = (int)pBallInfo.vptTrajectory.Count - (BALL_SPEED_INTERVAL - nFrameCountForSpeed[nBallIndex]) - 2; i < pBallInfo.vptTrajectory.Count - 2; i++)
				{
					...
					double fDistance = (double)(Math.Sqrt(((pt1.x - pt2.x) * (pt1.x - pt2.x) + (pt1.y - pt2.y) * (pt1.y - pt2.y))) * m_fMillPerDot);
					fTotalMovedDistance += fDistance;
				}

				double fTimeElapsed = (double)(nEndTime[nBallIndex] - nStartTime[nBallIndex]) / (double)1000;
				if (fTimeElapsed > 0.0f)
				{
					double fSpeed = fTotalMovedDistance / fTimeElapsed;
					if (fSpeed >= 1.0f)
					{
		...
		public static int FindDiamondMark(ref POINT ptCrashed, POINT ptPrev, int nSide)
		{
			...
			int nRet = 0;
			int nFocusValue = 0;
			int nStep = (int)((double)m_szCanvas.Width * 2.229166666666668f / 10.91666666666667f + 0.5f);
			int nOffset = 0;
			int nTableMargin = (int)((double)m_szCanvas.Width / 10.91666666666667f + 0.5f);
			int nDistToRail = (int)(nTableMargin * 0.3541666666666667f + 0.5f);
			double fScaleX = (double)(m_szCanvas.Width - nTableMargin * 2) / (double)(g_rcClip.bottom - g_rcClip.top);
			double fScaleY = (double)(m_szCanvas.Height - nTableMargin * 2) / (double)(g_rcClip.right - g_rcClip.left);
			switch (nSide)
			{
			case 1:
				{
					...
					nOffset = (int)((ptCrashed.x - ptPrev.x) * (double)nDistToRail / (double)(Math.Abs(ptCrashed.y - ptPrev.y)));
					ptCrashed.x += nOffset;

					if (ptCrashed.x < 0)
						ptCrashed.x = 0;
					if (ptCrashed.x > g_rcClip.Height())
						ptCrashed.x = g_rcClip.Height();
			...
			case 2:
				{
					...
					nOffset = (int)((ptCrashed.y - ptPrev.y) * (double)nDistToRail / (double)(Math.Abs(ptCrashed.x - ptPrev.x)));
					ptCrashed.y += nOffset;

					if (ptCrashed.y < 0)
						ptCrashed.y = 0;
					if (ptCrashed.y > g_rcClip.Width())
						ptCrashed.y = g_rcClip.Width();
			...
			for (int i = nFrom; i <= nTo - 1; i++)
			{
				...
				{
					double fRatio = (double)(nFocusValue - (i - nFrom) * nStep) / (double)nStep;
		...
		//get rail number
			...
			//08/14/2019, to check corner impact
			////check neighbor sides to guess player's try
			int nNeighborLeft = (nMinSideIndex - 1 + 4) % 4;
			int nNeighborRight = (nMinSideIndex + 1) % 4;

			int nLeftDist = pBallInfo.nDist[nNeighborLeft];
			int nRightDist = pBallInfo.nDist[nNeighborRight];
			int nCurDist = pBallInfo.nDist[nMinSideIndex];
			const int cnMinDist = 5;
			//
			if (!pBallInfo.bFirstInputPresented &&
				(nMinSideIndex == nInSide ||
				(Math.Abs(nLeftDist - nCurDist) <= cnMinDist && nInSide == nNeighborLeft) ||
				(Math.Abs(nRightDist - nCurDist) <= cnMinDist && nInSide == nNeighborRight)))
			{
				m_nTeacherMarkIn = FindDiamondMark(ref ptCrashExtended, ptPrev, nInSide);
				lpElement.tpi.nType = 1;
				lpElement.tpi.nPoint = m_nTeacherMarkIn;
				lpElement.tpi.nSide = nInSide;
			...
			nNeighborLeft = (nOutSide - 1 + 4) % 4;
			nNeighborRight = (nOutSide + 1) % 4;
		...
		{
			return (int)(Math.Sqrt((double)((pt1.x - pt2.x) * (pt1.x - pt2.x) + (pt1.y - pt2.y) * (pt1.y - pt2.y))) + 0.5f);
		}
		...
		public static bool CheckCurrentState()
		{
			try
			{
						...
						m_redBallInfo.nStableCount > nThreshold && m_nCurrentPlayer >= 0)
					{
						...
						if (!m_bStrikedTargetBall)
						{
							if (m_nCurrentPlayer == 0)
								m_whiteBallInfo.fAveSpeed = BTAPI_GetAveSpeedForRealTime(); // m_whiteBallInfo.fTotalSpeed / (double)m_whiteBallInfo.nSpeedRecordCount;
							else
								m_yellowBallInfo.fAveSpeed = BTAPI_GetAveSpeedForRealTime(); // (double)m_yellowBallInfo.nSpeedRecordCount;
						}
					}
				}
				...
				if (m_nCurrentPlayer == -1)
				{
					double nMoveTotalWhite = m_whiteBallInfo.nHorzMoveTotal * m_whiteBallInfo.nHorzMoveTotal + m_whiteBallInfo.nVertMoveTotal * m_whiteBallInfo.nVertMoveTotal + 1;
					double nMoveTotalYellow = m_yellowBallInfo.nHorzMoveTotal * m_yellowBallInfo.nHorzMoveTotal + m_yellowBallInfo.nVertMoveTotal * m_yellowBallInfo.nVertMoveTotal + 1;

					nMoveTotalWhite = Math.Sqrt(nMoveTotalWhite);
					nMoveTotalYellow = Math.Sqrt(nMoveTotalYellow);

					if (nMoveTotalWhite > 7 || nMoveTotalYellow > 7)
					{
		...
		public async static Task<bool> LoadShotHistory(string strTeacherName)
		{
            ...
            /**********Added for download from server************/
			string all_data = await Task.Run(() => API.downloadTeacherHistory(strTeacherName));
			if (all_data == false.ToString())
				return false;

			byte[] all_data_bin = Convert.FromBase64String(all_data);

			int meta_length, shot_length;

			meta_length = (all_data_bin[0] << 24) + (all_data_bin[1] << 16) + (all_data_bin[2] << 8) + all_data_bin[3];
			byte[] meta_data = new byte[meta_length];
			Array.Copy(all_data_bin, 4, meta_data, 0, meta_length);

			shot_length = (all_data_bin[meta_length + 4] << 24) + (all_data_bin[meta_length + 5] << 16) + (all_data_bin[meta_length + 6] << 8) + all_data_bin[meta_length + 7];
			byte[] shot_data = new byte[shot_length];
			Array.Copy(all_data_bin, meta_length + 8, shot_data, 0, shot_length);

            /****************************************************/

			FileStream fs = File.Open(strShotDataFile, FileMode.Create);
			BinaryWriter br = new BinaryWriter(fs);
			br.Write(shot_data, 0, shot_data.Length);
			br.Close();
			fs.Close();

            if (!ShotHistory.LoadFromMemory(meta_data, ref shotHistory))
                return false;
		...
		{
			if (nShotIndex < 0 || shotHistory.Shot.Count <= nShotIndex)
				return;
		...
		public static void DownloadFile(string remote_file, string file)
		{
            string fullUri = "http://track.biliardoprofessionale.it/download.aspx?file=" + remote_file;
			WebClient webClient = new WebClient();
			webClient.DownloadFile(fullUri, file);
		}
		...
		public async static Task<bool> SaveShotHistory(string strTeacherName)
		{
			string meta_data = ShotHistory.Convert2Base64(shotHistory);

			string strShotDataFile = Environment.CurrentDirectory + "/shot_temp.dat";
			if (!BallTrackAPI.BTAPI_SaveHistoryToFile(strShotDataFile))
				return false;

			FileStream fs = File.Open(strShotDataFile, FileMode.Open);
			BinaryReader br = new BinaryReader(fs);
			byte[] shot_data_bin = new byte[fs.Length];
			br.Read(shot_data_bin, 0, (int)fs.Length);
			string shot_data = Convert.ToBase64String(shot_data_bin);

			string result = await Task.Run(() => API.uploadTeacherHistory(strTeacherName, meta_data, shot_data));
            /*******************************************************/
			return true;
		}

        public async static Task<bool> SaveGameHistory(List<memorydetails> memory)
        {
            string gameid = BLL_BilliardWindowsApplication.gamecostdetailsStatic.id;

            string strDirHistory = Environment.CurrentDirectory;
            strDirHistory += "\\GameHistory";
            if (!Directory.Exists(strDirHistory))
                Directory.CreateDirectory(strDirHistory);
			...
			FileStream fs = File.Open(strShotDataFile, FileMode.Open);
			BinaryReader br = new BinaryReader(fs);
			byte[] game_data_bin = new byte[fs.Length];
			br.Read(game_data_bin, 0, (int)fs.Length);
			br.Close();
			fs.Close();
			string result = await Task.Run(() => API.uploadGameHistory(gameid, Convert.ToBase64String(game_data_bin)));
            return true;
        }
        /**************************************************************/
		public static void DrawReplay(Graphics g, int nStep, int nPlayer, POINT ptWhite, POINT ptYellow,
			...
			bool bShowBallSpeed, double fBallSpeed, int nImpactState, int nTeacherMarkIn, int nTeacherMarkOut, POINT ptTeacherIn, POINT ptTeacherOut)
		{
				...
				if (bDrawBlackDot && (nTeacherMarkIn >= 0 || nTeacherMarkOut >= 0))
				{
					...
					if (m_bShowTeacherPoint)
					{
						...
						if (nTeacherMarkOut >= 0)
						{
							...
							{
								if (m_ptMarkOut.X >= 0 && m_ptMarkOut.Y >= 0)
		...
		public static void BallSpeedDisplay(Graphics g, int left, int top, int nWidth, int nHeight, double fBallSpeed, int nImpactState, Color clrBackground)
		{
            if (fBallSpeed <= 0.0f)
                return;
		...
		public static void DrawTrajectory(Graphics g, BALL_TRACK_INFO pBallInfo, int nStep, double fScaleX, double fScaleY, int nOffsetX, int nOffsetY, int nColorIndex)
        {
            if (pBallInfo.vptTrajectory.Count() <= 0)
                return;
                    ...
                    for (int i = 0; i < (int)pBallInfo.vIndexCrashed.Count(); i++)
                    {
                        ...
                        if (m_bShowTeacherPoint)
                        {
                        	...
                        	else if (pBallInfo.vIndexCrashed[i].tpi.nType == 2)
                        	{
								...
								if (ptTracjectory.X < 0 || ptTracjectory.Y < 0)
									continue;
                        		ptTracjectory.Offset(nOffsetY, nOffsetY);
	...
	public class BALL_TRACK_INFO
	{
		...
		public double fCurrentSpeed;
		public double fTotalSpeed;
		public double fAveSpeed;
